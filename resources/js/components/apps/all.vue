<template>
  <main role="main" class="main-content">
    ...
    <div class="container-fluid">
      <h2 class="h5 page-title pb-5">كل التطبيقات البرمجية</h2>
      ...
      <table class="table mt-5 table-hover">
        ...
        <tbody>
          <tr v-for="(app, index) in apps" :key="app.id">
            ...
            <td>{{ app.title }}</td>
            <td>
              <img :src="app.image" width="100" height="70" alt="" />
            </td>
            <td>{{ app.c_name }}</td>
            <td class="actions">
              <router-link :to="{ name: 'edit_app', params: { id: app.id } }">
                <button type="button"><i class="fe fe-edit fe-16"></i></button
              ></router-link>
              <button type="button" @click="delCat(app.id)">
...
export default {
  ...
  methods: {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            axios.post(`/api/dash/app/destroy/${id}`);
    ...
    async fetchapps(page_url) {
      this.loading = true;
      page_url = page_url || `api/dash/apps`;
      await axios
        .get(page_url)
        .then((res) => {
